...
void paging_init(void) {
    ...
    // Temporarily use the firmware's identity map as the offset map
    page_table[(IDENTITY_MAP_OFFSET >> 39) & 0x1FF] = page_table[0];

    // Offset to use the linear mapping created above;
    page_table = (uint64_t*)((uint64_t)page_table + IDENTITY_MAP_OFFSET);

    // Map ALL of the ram linearly at a large offset
    // (Also replaces the workaround done above with a longer term map)
    identity_map_memory();
...
uint64_t* paging_get_pte_address(uint64_t virtual_address, uint64_t* table_base) {
    ...
    uint64_t* table = table_base;

    if (((uint64_t)table_base & 0xfff) != 0) {
        tty_print_string("\nWARNING: table_base not page aligned.");
    }

    if (entry_present(table[page_level_4_index]) == 0) {
        tty_print_string("\npml4 entry not present");
        return 0; // Return null
    }

    // Get the page directory pointer's address 
    table = (uint64_t*)(extract_entry_address(table[page_level_4_index]) + IDENTITY_MAP_OFFSET);
    ...
    table = (uint64_t*)((uint64_t)table - IDENTITY_MAP_OFFSET);

    if ((uint64_t)table > IDENTITY_MAP_OFFSET) {
        tty_print_string("ERROR");
    }
...
uint64_t paging_get_physical_address(uint64_t virtual_address) {
    ...
    // Chain calls to allow outside files to get physcial addresses without requiring a table pointer
    return paging_get_physical_address_from_table(virtual_address, page_table);
}
...
uint64_t paging_get_physical_address_from_table(uint64_t virtual_address, uint64_t* table_base) {
    ...
    // Get the lowest level page table entry's address, and offset it to the linearly mapped RAM so it can be accessed by the code
    // Typecasts used to prevent pointer arithmatic from messing up the address
    // TODO: Can this be done without typecasts? 
    uint64_t* address = (uint64_t*) ((uint64_t)paging_get_pte_address(virtual_address, table_base) + IDENTITY_MAP_OFFSET);
...
// Map a virtual address page to a page of physical RAM
void paging_map_page(uint64_t virtual_address, uint64_t physical_address, uint16_t flags, uint64_t* table_base) {
    ...
    uint8_t is_user = (flags & 0b100) >> 2; // If the page is a user page, every table entry above it must also be labeled as user
    uint8_t is_writable = (flags & 0b10) >> 1; // If the page is writable, every entry above it must also be writable
    ...
    if (entry_present(table_base[page_level_4_index]) == 0) {
        // Allocate a new table level, put the address in the table, and mark it as present
        paging_set_entry(&table_base[page_level_4_index], paging_get_physical_address((uint64_t)paging_allocate_table_level()), flags);
        tty_print_string("Allocated new PDP\n");
    }

    // If the page being mapped is a user or writable page, mark the higher levels with the right flags as well
    if (is_user != 0)     { table_base[page_level_4_index] |= 0b100; }
    if (is_writable != 0) { table_base[page_level_4_index] |= 0b10; }

    // Store a pointer to the next level of the table
    table_pointer = (uint64_t*)(extract_entry_address(table_base[page_level_4_index]) + IDENTITY_MAP_OFFSET);
    ...
    if (entry_present(table_pointer[page_directory_pointer_index]) == 0) {
        // Allocate a new table level, put the address in the table, and mark it as present
        paging_set_entry(&table_pointer[page_directory_pointer_index], paging_get_physical_address((uint64_t)paging_allocate_table_level()), flags);
        tty_print_string("Allocated new PD\n");
    }

    // If the page being mapped is a user or writable page, mark the higher levels with the right flags as well
    if (is_user != 0)     { table_pointer[page_directory_pointer_index] |= 0b100; }
    if (is_writable != 0) { table_pointer[page_directory_pointer_index] |= 0b10; }
    ...
    if (entry_present(table_pointer[page_directory_index]) == 0) {
        // Allocate a new table level, put the address in the table, and mark it as present
        paging_set_entry(&table_pointer[page_directory_index], paging_get_physical_address((uint64_t)paging_allocate_table_level()), flags);
        tty_print_string("Allocated new PT\n");
    }

    // If the page being mapped is a user or writable page, mark the higher levels with the right flags as well
    if (is_user != 0)     { table_pointer[page_directory_index] |= 0b100; }
    if (is_writable != 0) { table_pointer[page_directory_index] |= 0b10; }
    ...
    // Clear the tlb cache for this page
    invlpg((uint64_t*)virtual_address);
}

// Unmap a page of virtual memory. Skips some of the overhead of paging_map_page
void paging_unmap_page(uint64_t virtual_address, uint64_t* table_base) {
    
    // Get the page entry address
    uint64_t* entry = paging_get_pte_address(virtual_address, table_base);
...
void paging_allocate_kernel_pages(void * start_address, uint64_t count, uint16_t flags) {
    ...
    for (uint64_t i = 0; i < count; i++) {
        paging_map_page(virtual_address, physcial_address, flags, page_table);
...
void paging_allocate_user_pages(void * start_address, uint64_t count, uint16_t flags, uint64_t* address_space_pml4) {
    ...
    uint64_t virtual_address = (uint64_t)start_address; 
    uint16_t level_4_index = (virtual_address >> 39) & 0x1FF; // The pml4 index the mapping will occur in

    // If the mapping occurs in the first or second entry,
    if (level_4_index < 2) {
        // Allocate the physical memory
        uint64_t physcial_address = allocate_range(count*4096);
        ...
        // Map every page of the allocated memory
        for (uint64_t i = 0; i < count; i++) {
            paging_map_page(virtual_address, physcial_address, flags, address_space_pml4);
            // Increment the addresses by a single page
            virtual_address  += 4096;
            physcial_address += 4096;
        }
...
void * paging_create_address_space(void) {
    ...
    // Copy the original top level table to the newly allocated table
...
void paging_load_process_address_space(void * table_base_address) {
    ...
    // TODO: Find a better way to store process pml4s (So that a full page is not used for 4 entries)
    memcpy(page_table, table_base_address, 32); // Copy the first 4 entries

    // Reload the cr3 register to clear the cache 
